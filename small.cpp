...
int main() {
	...
	// layer[1].push_back(u.rotated(pi / 2));
	// layer[2].push_back(layer[1][0] + u.rotated(pi / 2 - deg(70)));
	// layer[2].push_back(layer[1][0] + u.rotated(pi / 2 + deg(70)));
...
pair d[][];
	...
	// Polygon<ld> region({layer[2][0] + layer[2][0] / layer[2][0].len() * 10, layer[2][0], layer[1][0], layer[2][1], layer[2][1] + layer[2][1] / layer[2][1].len() * 10});
	...
	vector<pair<int, int>> seq = {{1, 0}, {0, 0}, {1, 1}};
	// vector<pair<int, int>> seq = {{2, 0}, {1, 0}, {2, 1}};
	{
		decltype(seq) tmp;
		for (auto p : seq) {
